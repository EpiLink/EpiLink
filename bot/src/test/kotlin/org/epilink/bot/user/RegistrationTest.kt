...
class RegistrationTest : TegralSubjectTest<Unit>(
    Unit::class,
    {
    ...
    @Test
    fun `Test Microsoft account authcode registration`() = test {
    ...
    @Test
    fun `Test Microsoft account authcode registration when disallowed`() = test {
        putMock<IdentityProvider> {
            coEvery { getUserIdentityInfo("fake mac", "fake mur") } returns UserIdentityInfo("fakeguid", "fakemail")
        }
        putMock<PermissionChecks> {
    ...
    @Test
    fun `Test Discord account authcode registration account does not exist`() = test {
    ...
    @Test
    fun `Test Discord account authcode registration account already exists`() = test {
        putMock<DiscordBackEnd> {
            coEvery { getDiscordToken("fake auth", "fake uri") } returns "fake yeet"
            coEvery { getDiscordInfo("fake yeet") } returns DiscordUserInfo("yes", "no", "maybe")
        }
    ...
    @Test
    fun `Test Discord account authcode registration when disallowed`() = test {
        putMock<DiscordBackEnd> {
            coEvery { getDiscordToken("fake auth", "fake uri") } returns "fake yeet"
            coEvery { getDiscordInfo("fake yeet") } returns DiscordUserInfo("yes", "no", "maybe")
        }
    ...
    @Test
    fun `Test registration session deletion`() = test {
        withTestEpiLink {
    ...
    fun `Test full registration sequence, discord then msft`() = test {
