...
class PermissionChecksTest : TegralSubjectTest<PermissionChecks>(
    PermissionChecks::class,
    { put<PermissionChecks>(::PermissionChecksImpl) }
) {
    @Test
    fun `Test Discord user who already exists cannot create account`() = test {
    ...
    @Test
    fun `Test Discord user who does not exist should be able to create account`() = test {
        putMock<DatabaseFacade> {
    ...
    @Test
    fun `Test Microsoft user who already exists cannot create account`() = test {
        val hey = "hey".sha256()
        putMock<DatabaseFacade> {
    ...
    @Test
    fun `Test banned Microsoft user cannot create account`() = test {
        val hey = "hey".sha256()
        putMock<DatabaseFacade> {
    ...
    @Test
    fun `Test Microsoft user with no validator can create account`() = test {
        val hey = "hey".sha256()
        putMock<DatabaseFacade> {
            coEvery { isIdentityAccountAlreadyLinked(hey) } returns false
    ...
    @Test
    fun `Test Microsoft user with email rejected cannot create account`() = test {
        val hey = "hey".sha256()
        putMock<DatabaseFacade> {
            coEvery { isIdentityAccountAlreadyLinked(hey) } returns false
        }
    ...
    @Test
    fun `Test indefinitely banned user cannot join servers`() = test {
    ...
    @Test
    fun `Test normal user can join servers`() = test {
        val hey = "tested".sha256()
        putMock<DatabaseFacade> {
            coEvery { getBansFor(hey) } returns listOf()
        }
    ...
    @Test
    fun `Test admin not an admin check fails`() = test {
    ...
    @OptIn(UsesTrueIdentity::class)
    @Test
    fun `Test admin not identifiable check fails`() = test {
    ...
    @OptIn(UsesTrueIdentity::class)
    @Test
    fun `Test admin check success`() = test {
        val user = mockk<User> { every { discordId } returns "notyou" }
        put(named("admins")) { listOf("notme", "notyou") }
        putMock<DatabaseFacade> {
