...
class IDPTest : TegralSubjectTest<IdentityProvider>(
    ...
    @Test
    fun `Test user identity retrieval`() = test {
    ...
    @Test
    fun `Test user identity info retrieval failure invalid_grant`() = test {
        declareTokenRequestHandler("""{"error": "invalid_grant"}""", HttpStatusCode.BadRequest)

        val ex = assertFailsWith<UserEndpointException> { subject.getUserIdentityInfo("AUTHCODE", "REDIRECT_URI") }
    ...
    @Test
    fun `Test user identity info retrieval failure other`() = test {
    ...
    @Test
    fun `Test user identity info retrieval no token returned`() = test {
    ...
    @Test
    fun `Test user identity info retrieval invalid token`() = test {
        declareTokenRequestHandler("""{"id_token":"JWT_TOKEN"}""")
        val ex = mockk<InvalidJwtException>()
        putMock<JwtVerifier> { coEvery { process("JWT_TOKEN") } throws ex }

        val ex2 = assertFailsWith<InvalidJwtException> { subject.getUserIdentityInfo("AUTHCODE", "REDIRECT_URI") }
        assertSame(ex, ex2)
    }
...
private fun TestMutableInjectionEnvironment.declareTokenRequestHandler(
    response: String,
