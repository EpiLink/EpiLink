...
internal class DiscordCommandsImpl(scope: InjectionScope) : DiscordCommands {
    ...
    private val commands by scope<List<Command>>(named("discord.commands"))

    private val commandsByName by lazy { commands.associateBy { it.name } }
    ...
    private suspend fun shouldAcceptMessage(message: String, senderId: String, serverId: String?): MessageAcceptStatus {
        ...
        val command = commandsByName[name]
        return when {
            command == null -> {
                UnknownCommand(name)
            }

            monitoringMismatch(command, serverId) -> {
                ServerNotMonitored
            }
