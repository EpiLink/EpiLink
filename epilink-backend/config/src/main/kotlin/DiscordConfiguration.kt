 ...
 * - Unused rules
 */
fun DiscordConfiguration.checkCoherenceWithRulebook(rulesDeclaredInRulebook: Set<String>): List<ConfigReportElement> {
    val report = mutableListOf<ConfigReportElement>()
    val ruleNamesUsedInServers =
        servers.flatMap { it.requires.map { ruleName -> ruleName to it } }
            .groupBy({ it.first }, { it.second.id })

    val missingRules = ruleNamesUsedInServers - rulesDeclaredInRulebook
    ...
    val unusedRules = rulesDeclaredInRulebook - ruleNamesUsedInServers.keys
