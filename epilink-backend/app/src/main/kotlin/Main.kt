...
import org.epilink.backend.config.ConfigError
import org.epilink.backend.config.ConfigInfo
import org.epilink.backend.config.ConfigWarning
import org.epilink.backend.config.Configuration
import org.epilink.backend.config.isConfigurationSane
import org.epilink.backend.config.loadConfigFromFile
import org.epilink.backend.http.IdentityProviderMetadata
import org.epilink.backend.http.MetadataOrFailure
import org.epilink.backend.http.identityProviderMetadataFromDiscovery
import org.epilink.backend.rulebook.Rulebook
import org.epilink.backend.rulebook.loadRules
import org.epilink.backend.rulebook.loadRulesWithCache
import org.epilink.backend.rulebook.readScriptSource
import org.epilink.backend.version.EPILINK_VERSION
...
fun launchEpiLink(cliArgs: CliArgs) = runBlockingWithScope {
    ...
    checkConfig(cfg, rulebook.await(), strings.await().keys)
...
private fun checkConfig(
    ...
    val configReport = cfg.isConfigurationSane(rulebook.rules.keys, availableDiscordLanguages)
...
private suspend fun download(url: String) = HttpClient(Apache).get<String>(url)
