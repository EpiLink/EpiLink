...
internal class DiscordTargetsImpl : DiscordTargets, KoinComponent {
    ...
    override fun parseDiscordTarget(target: String): TargetParseResult {
        ...
        if (angled != null) {
            ...
            return if (isRole) TargetParseResult.Success.RoleById(id) else TargetParseResult.Success.UserById(id)
        } else {
            return when (target.first()) {
                '|' -> TargetParseResult.Success.RoleByName(target.drop(1))
                '/' -> TargetParseResult.Success.RoleById(target.drop(1))
                '!' ->
                    if (target == "!everyone") {
                        TargetParseResult.Success.Everyone
                    } else TargetParseResult.Error
                else ->
                    if (target.matches(Regex("\\d+"))) TargetParseResult.Success.UserById(target)
                    else TargetParseResult.Error
            }
        }
    }
