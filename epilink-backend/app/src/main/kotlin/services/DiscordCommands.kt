...
interface DiscordCommands : DiscordMessageHandler
...
internal class DiscordCommandsImpl : DiscordCommands, KoinComponent {
    ...
    override suspend fun handleMessage(message: String, senderId: String, channelId: String, serverId: String?) {
        when (val a = shouldAcceptMessage(message, senderId, serverId)) {
            MessageAcceptStatus.NotACommand -> {
                // return silently
            }
            ...
            MessageAcceptStatus.NotRegistered ->
            ...
            MessageAcceptStatus.AdminNoIdentity ->
            ...
            MessageAcceptStatus.ServerNotMonitored ->
            ...
            is MessageAcceptStatus.UnknownCommand ->
    ...
    private suspend fun shouldAcceptMessage(message: String, senderId: String, serverId: String?): MessageAcceptStatus {
        if (!message.startsWith(discordCfg.commandsPrefix)) {
            return MessageAcceptStatus.NotACommand
        }
        // Get the command
        ...
        return if (command == null) {
            MessageAcceptStatus.UnknownCommand(name)
            ...
            PermissionLevel.User -> {
                ...
                if (user == null) MessageAcceptStatus.NotRegistered else MessageAcceptStatus.Accept(user, command, body)
            }
            PermissionLevel.Anyone -> MessageAcceptStatus.Accept(db.getUser(senderId), command, body)
        }
    }
    ...
    private suspend fun checkAdminCommandAllowed(senderId: String, command: Command, body: String) =
        ...
        if (senderId !in admins) {
            ...
            if (user == null) {
                ...
                AdminStatus.NotAdmin -> MessageAcceptStatus.NotAdmin
                AdminStatus.AdminNotIdentifiable -> MessageAcceptStatus.AdminNoIdentity
                AdminStatus.Admin -> {
                    MessageAcceptStatus.Accept(user, command, body)
                }
            }
        }
}
