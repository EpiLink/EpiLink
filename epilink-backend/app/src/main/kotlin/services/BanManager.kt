...
internal class BanManagerImpl : BanManager, KoinComponent {
    ...
    override suspend fun revokeBan(idpHashBase64: String, banId: Int) {
        val actualHash = Base64.getUrlDecoder().decode(idpHashBase64)
        val ban =
            dbf.getBan(banId) ?: throw UserEndpointException(StandardErrorCodes.InvalidId, "Unknown ban ID", "bm.ubi")
        if (!ban.idpIdHash.contentEquals(actualHash)) {
            throw UserEndpointException(
                StandardErrorCodes.InvalidId,
                "Identity Provider ID hash does not match given ban ID",
                "bm.mid"
            )
        }
