...
import org.epilink.backend.http.admin
import org.epilink.backend.http.apiSuccess
import org.epilink.backend.http.data.BanInfo
import org.epilink.backend.http.data.BanRequest
import org.epilink.backend.http.data.IdRequest
import org.epilink.backend.http.data.IdRequestResult
import org.epilink.backend.http.data.RegisteredUserInfo
import org.epilink.backend.http.data.UserBans
...
import org.epilink.backend.toResponse
...
internal class AdminEndpointsImpl : AdminEndpoints, KoinComponent {
    ...
    private fun Route.user() {
        ...
        get<UserByTargetId> { userByTargetId ->
            ...
            if (user == null) {
                call.respond(NotFound, StandardErrorCodes.TargetUserDoesNotExist.toResponse())
            } else {
    ...
    private fun Route.ban() {
        ...
        postL<BansByIdpHash> { banByIdHash ->
            ...
            if (expiry == null && request.expiresOn != null) {
                call.respond(
                    BadRequest,
                    StandardErrorCodes.InvalidInstant.toResponse("Invalid expiry timestamp.", "adm.iet")
                )
            } else {
                val admin = call.admin
        ...
        get<BansByIdpHash.Ban> { banLocation ->
            ...
            if (banId == null) {
                call.respond(BadRequest, StandardErrorCodes.InvalidId.toResponse("Invalid ban ID.", "adm.ibi"))
            } else {
                ...
                when {
                    ban == null ->
                        call.respond(
                            NotFound,
                            StandardErrorCodes.InvalidId.toResponse("No ban with given ID found.", "adm.nbi")
                        )
                    !ban.idpIdHash.contentEquals(idpHashBytes) ->
                        call.respond(
                            NotFound,
                            StandardErrorCodes.InvalidId.toResponse(
        ...
        postL<BansByIdpHash.Ban.Revoke> { revoke ->
            ...
            if (banId == null) {
                call.respond(BadRequest, StandardErrorCodes.InvalidId.toResponse("Invalid ban ID format", "adm.ibi"))
            } else {
    ...
    private fun Route.search() {
        get<SearchByHash> { location ->
            ...
            if (targetId.any { it !in hexCharacters }) {
                call.respond(
                    BadRequest,
                    StandardErrorCodes.InvalidAdminRequest.toResponse("Invalid hex string", "adm.ihs")
                )
            } else {
