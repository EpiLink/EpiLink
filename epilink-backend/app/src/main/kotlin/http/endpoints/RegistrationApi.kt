...
import org.epilink.backend.http.data.AdditionalRegistrationOptions
import org.epilink.backend.http.data.RegistrationAuthCode
import org.epilink.backend.http.data.RegistrationContinuation
import org.epilink.backend.http.data.RegistrationInformation
import org.epilink.backend.http.sessions.RegisterSession
...
internal class RegistrationApiImpl : RegistrationApi, KoinComponent {
            ...
            with(call.sessions.get<RegisterSession>()) {
                if (this == null) {
                    ...
                    call.respond(
                        ...
                        StandardErrorCodes.IncompleteRegistrationRequest.toResponse(
            ...
            when (val service = call.parameters["service"]) {
                ...
                "idProvider" -> processIdProviderAuthCode(call, call.receive())
                    ...
                    call.respond(
                        ...
                        StandardErrorCodes.UnknownService.toResponse(
    ...
    private suspend fun processIdProviderAuthCode(call: ApplicationCall, authcode: RegistrationAuthCode) {
        ...
        if (adv is DatabaseAdvisory.Disallowed) {
            logger.debug { "IDP user $id ($email) is not allowed to create an account: " + adv.reason }
            call.respond(
                HttpStatusCode.BadRequest,
                StandardErrorCodes.AccountCreationNotAllowed.toResponse(adv.reason, adv.reasonI18n, adv.reasonI18nData)
            )
            return
        }
    ...
    private suspend fun processDiscordAuthCode(call: ApplicationCall, authcode: RegistrationAuthCode) {
        ...
        if (user != null) {
            ...
            if (adv is DatabaseAdvisory.Disallowed) {
                ...
                call.respond(
                    HttpStatusCode.BadRequest,
                    StandardErrorCodes.AccountCreationNotAllowed.toResponse(
                        adv.reason,
                        adv.reasonI18n,
                        adv.reasonI18nData
                    )
