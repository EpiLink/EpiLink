...
import org.epilink.backend.fromJson
import org.epilink.backend.getMap
import org.epilink.backend.getString
...
import org.epilink.backend.setJsonBody
...
class RegistrationTest : KoinBaseTest<Unit>(
    ...
    fun `Test Microsoft account authcode registration`() {
        ...
        mockHere<PermissionChecks> {
            coEvery { isIdentityProviderUserAllowedToCreateAccount(any(), any()) } returns DatabaseAdvisory.Allowed
        }
        withTestEpiLink {
    ...
    fun `Test Microsoft account authcode registration when disallowed`() {
        ...
        mockHere<PermissionChecks> {
            ...
            } returns DatabaseAdvisory.Disallowed("Cheh dans ta tronche", "ch.eh")
        }
        withTestEpiLink {
            val call = handleRequest(HttpMethod.Post, "/api/v1/register/authcode/idProvider") {
                setJsonBody("""{"code":"fake mac","redirectUri":"fake mur"}""")
    ...
    fun `Test Discord account authcode registration account does not exist`() {
        ...
        mockHere<PermissionChecks> {
            coEvery { isDiscordUserAllowedToCreateAccount(any()) } returns DatabaseAdvisory.Allowed
        }
    ...
    fun `Test Discord account authcode registration when disallowed`() {
        ...
        mockHere<PermissionChecks> {
            coEvery { isDiscordUserAllowedToCreateAccount(any()) } returns DatabaseAdvisory.Disallowed("Cheh dans ta tête", "ch.eh2")
        }
        mockHere<DatabaseFacade> {
            coEvery { getUser("yes") } returns null
        }
